if not lib.checkDependency("ox_lib", "3.30.0", true) then
	return
end
...
local function startTargeting()
	if state.isDisabled() or state.isActive() or IsNuiFocused() or IsPauseMenuActive() then
		return
	end
...
local function getResponse(option, server)
	...
	if server then
		response.entity = response.entity ~= 0
